...
from .transformer_2dmv import Transformer2DMVModel
...
def get_down_block(
    ...
    dropout: float = 0.0,
    additional_unet_kwargs: Optional[Dict[str, Any]] = None,
):
    ...
    if down_block_type == "DownBlock2D":
        ...
        return CrossAttnDownBlock2D(
            ...
            additional_unet_kwargs=additional_unet_kwargs,
        )
...
class UNetMidBlock2DCrossAttn(nn.Module):
    def __init__(
        ...
        self.use_mv = additional_unet_kwargs["multiview_self_attn"]
        ...
        for i in range(num_layers):
            if additional_unet_kwargs["multiview_self_attn"]:
                attentions.append(
                    Transformer2DMVModel(
                        num_attention_heads,
            ...
            lighting_modules.append(
                ...
                if additional_unet_kwargs["lighting_cross_attn"]
                else None
            )
            resnets.append(
                ResnetBlock2D(
        ...
        self.attentions = nn.ModuleList(attentions)
        self.resnets = nn.ModuleList(resnets)
        self.lighting_modules = nn.ModuleList(lighting_modules)
    ...
    def forward(
        ...
        hidden_states = self.resnets[0](hidden_states, temb)
        for attn, resnet, lighting in zip(self.attentions, self.resnets[1:], self.lighting_modules):
            if self.training and self.gradient_checkpointing:
                ...
                hidden_states = attn(
                    hidden_states,
                    encoder_hidden_states=encoder_hidden_states if not self.use_mv else None,
                    cross_attention_kwargs=cross_attention_kwargs,
                    attention_mask=attention_mask,
                    encoder_attention_mask=encoder_attention_mask,
                    return_dict=False,
                )[0]
                hidden_states = lighting(hidden_states, encoder_hidden_states=encoder_hidden_states, return_dict=False)[0] if lighting is not None else hidden_states
...
class CrossAttnDownBlock2D(nn.Module):
    ...
    def forward(
        ...
        blocks = list(zip(self.resnets, self.attentions, self.lighting_modules))

        for i, (resnet, attn, lighting) in enumerate(blocks):
            if self.training and self.gradient_checkpointing:
...
class CrossAttnUpBlock2D(nn.Module):
    ...
    def forward(
        ...
        for resnet, attn, lighting in zip(self.resnets, self.attentions, self.lighting_modules):
            # pop res hidden states
            res_hidden_states = res_hidden_states_tuple[-1]
            res_hidden_states_tuple = res_hidden_states_tuple[:-1]
