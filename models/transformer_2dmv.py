...
from .MultiViewTransformer import MultiViewTransformerBlock

logger = logging.get_logger(__name__)  # pylint: disable=invalid-name


class Transformer2DMVModelOutput(Transformer2DModelOutput):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)


class Transformer2DMVModel(LegacyModelMixin, LegacyConfigMixin):
    """
    ...
    def __init__(
        ...
        # 1. Transformer2DMVModel can process both standard continuous images of shape `(batch_size, num_channels, width, height)` as well as quantized image embeddings of shape `(batch_size, num_image_vectors)`
    ...
    def _init_continuous_input(self, norm_type):
        ...
        self.transformer_blocks = nn.ModuleList(
            [
                MultiViewTransformerBlock(BasicTransformerBlock(
                    self.inner_dim,
    ...
    def _init_vectorized_inputs(self, norm_type):
        assert self.config.sample_size is not None, "Transformer2DMVModel over discrete input must provide sample_size"
        assert (
            self.config.num_vector_embeds is not None
        ), "Transformer2DMVModel over discrete input must provide num_embed"
    ...
    def _init_patched_inputs(self, norm_type):
        assert self.config.sample_size is not None, "Transformer2DMVModel over patched input must provide sample_size"
    ...
    def forward(
        ...
        """
        The [`Transformer2DMVModel`] forward method.
        ...
        Returns:
            If `return_dict` is True, an [`~models.transformers.transformer_2d.Transformer2DMVModelOutput`] is returned,
        ...
        return Transformer2DMVModelOutput(sample=output)
